...
export default function TableManagement() {
  ...
  const { data: tables = [], isLoading } = useQuery({
    ...
    queryFn: () => {
      // TODO: 백엔드 연동 시 fetchTables(adminUser.storeId) 사용
      // 임시 Mock 데이터
      return Promise.resolve([
        { tableId: 1, tableNumber: 1, isActive: true, sessionId: 'session-1' },
        { tableId: 2, tableNumber: 2, isActive: false, sessionId: null },
      ]);
    },
          ...
          <TableBody>
            {tables.map(table => (
              <TableRow key={table.tableId}>
                ...
                <TableCell>{table.isActive ? '활성' : '비활성'}</TableCell>
                <TableCell>{table.sessionId || '-'}</TableCell>
                <TableCell align="right">
                  <Button
                    size="small"
                    onClick={() => handleShowHistory(table.tableId)}
                    sx={{ mr: 1 }}
                  >
                    과거 내역
                  </Button>
                  {table.isActive && (
                    <Button
                      size="small"
                      color="warning"
                      onClick={() => handleEndSession(table.tableId)}
                    >
                      매장 이용 완료
                    </Button>
                  )}
